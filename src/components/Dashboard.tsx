...
import { calculateTeamStats, countMatchesAndSuperOvers, countMatchesPerSeason, countTossDecisionsPerTeam} from '@/lib/helper';
...
const Dashboard= () => {
  ...
  return (
    <main className="flex flex-col h-screen overflow-y-auto rounded">
      ...
      <div className='grid  grid-cols-1 xl:grid-cols-2 w-full gap-4 p-4 md:gap-8 items-baseline'>
...
<div className="overflow-auto w-full px-4 border-y-2">
  <table className="min-w-full bg-white text-sm">
    <thead className="text-left">
      <tr>
        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-600">Team</th>
        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-600">Wins</th>
        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-600">Loss</th>
        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-600">Average Runs per match</th>
        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-600">Matches Played</th>
      </tr>
    </thead>
    ...
    <tbody className="">
      ...
      <tr key={index}>
        <td className="whitespace-nowrap px-4 py-4 text-md font-semibold text-black">{item.team}</td>
        <td className="whitespace-nowrap px-4 py-4 text-md text-green-900"><span className='bg-emerald-500/30 px-2 rounded-full'>{item.wins}</span></td>
        <td className="whitespace-nowrap px-4 py-4 text-md text-red-9;00"><span className='bg-red-500/30 px-2 rounded-full'>{item.losses}</span></td>
        <td className="whitespace-nowrap px-4 py-4 text-md text-gray-700">{item.avgRunsPerMatch}</td>
        <td className="whitespace-nowrap px-4 py-4 text-md text-gray-700">{item.matchesPlayed}</td>
      </tr>
          )
        })
      }
...
<Card className=''>
          ...
          <CardContent>
            <div className="text-4xl font-bold">{matchSuper.totalMatches}</div>
        ...
        <Card className=''>
          ...
          <CardContent>
            <div className="text-4xl font-bold">{matchSuper.superOvers}</div>
            <div className="text-sm text-muted-foreground"></div>
          </CardContent>
        </Card>
